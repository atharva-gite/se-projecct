...
class Portfolios {
  ...
  createPortfolio = async (req, res) => {
    try {
      ...
      const existingPortfolio = await Portfolio.findOne({ owner: userId, name });
  
      if (existingPortfolio) {
        // Redirect to the error view
        return res.render('errorView', { error: "Portfolio already exists with the same name" });
      }
  
      const user = await User.findById(userId);
      const newPortfolio = new Portfolio({ name, owner: userId });
      user.portfolios.push(newPortfolio._id);
  
      await Promise.all([newPortfolio.save(), user.save()]);
  
      // Redirect to the success view
      res.render('successView', { success: "Portfolio created successfully" });
  
    } catch (error) {
      console.error(error);
      // Redirect to the error view for internal server errors
      res.render('errorView', { error: "Internal Server Error" });
    }
  };
  ...
  deletePortfolio = async (req, res) => {
    ...
    req.flash('success', `Successfully deleted the portfolio "${portfolio.name}" !`);
